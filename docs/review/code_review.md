...
- Did it relax the strict paired-sample contract or add silent pair repair?
...
- Did validation actually check fixed `(CC, MLO)` ordering and probability export semantics where relevant?
...
- Did config comments or examples become stale?
...
- Could `CC` / `MLO` semantics have been swapped while the code still runs?
- Could exported `prediction` values be logits even though docs claim they are probabilities?
